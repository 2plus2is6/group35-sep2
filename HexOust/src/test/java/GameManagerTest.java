...
public class GameManagerTest {
    ...
    public static void initJavaFX() {
        JavaFXTestInitializer.initialize();
    }
    ...
    void testRedWinningCondition() throws InterruptedException {
        // Place Blue stones on the board
        board.getHexStatus()[6][6] = "Blue"; // Center (q=0, r=0)
        board.getHexStatus()[7][6] = "Blue"; // East (q=1, r=0)

        // Simulate a move to ensure opponentHadStones is set to true
        // Place a Red stone at a different location to trigger hasPlayerWon
        double initialClickX = 410 - 30 * 1.5; // West (q=-1, r=0)
        double initialClickY = 345;
        gameManager.makeMove(gc, initialClickX, initialClickY); // Valid move, sets opponentHadStones = true

        // Red surrounds the Blue stones to capture them
        ...
        board.getHexStatus()[5][6] = "Red"; // West (q=-1, r=0), already placed

        // Simulate Red placing the final stone to capture Blue stones (Southeast, q=1, r=1)
        double finalClickX = 410 + 30 * 1.5; // Southeast X
        double finalClickY = 345 + 30 * Math.sqrt(3) / 2; // Southeast Y
        CountDownLatch latch = new CountDownLatch(1);
        Platform.runLater(() -> {
            try {
                gameManager.makeMove(gc, finalClickX, finalClickY); // Should capture Blue stones
                // After capture, Blue should have no stones left
    ...
    void testBlueWinningCondition() throws InterruptedException {
        ...
        // Blue surrounds the Red stones to capture them
        ...
        board.getHexStatus()[5][6] = "Blue"; // West (q=-1, r=0), already placed

        // Simulate Blue placing the final stone to capture Red stones (Southeast, q=1, r=1)
        double finalClickX = 410 + 30 * 1.5; // Southeast X
        double finalClickY = 345 + 30 * Math.sqrt(3) / 2; // Southeast Y
        CountDownLatch latch = new CountDownLatch(1);
        Platform.runLater(() -> {
            try {
                gameManager.makeMove(gc, finalClickX, finalClickY); // Should capture Red stones
                // After capture, Red should have no stones left
                assertTrue(renderer.getWinMessageLabel().isVisible(), "Win message should be displayed");
